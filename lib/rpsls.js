...
export {rps, rpsls}; // Make sure to have the following function names (Documentation: this module needs to be importable)
//export {rpsls}; // Simply list this export above using another comma to simply syntax
...
function rps(move){
    ...
    const potential_choice = ['rock', 'paper', 'scissors']; // Does it matter if I use single or double quotes?
    ...
    if (move === undefined) { // Following would deal with #4 operational requirement ('===' does a stricter comparison between operands)
        ...
        console.error('Error: The following move is not allowed. The only options you can select are rock, paper, or scissors!!')
        ...
        switch (move.toLowerCase() ){ // Change spacing here
            // Now list the different cases (order doesn't necessarily matter here)
            ...
            case 'scissors':
                ...
                outcome = (opponent_move === 'paper') ? 'win':
                          ...
                          break;
            case 'rock':
                // Same structure as above (copy from above case) | Does it matter if we use '' instead of ""?
        ...
        return {
            ...
            result: outcome // Use 'result' again here?
        };
...
// RPSLS Game 
function rpsls(move){ // Copy method from above and make a few changes to fit the new game
    // Create an array of Strings representing all the choices that can be made
    const potential_choice = ['rock', 'paper', 'scissors', 'lizard', 'spock']; // Does it matter if I use single or double quotes?
    // Create a variable representing a random move that a computer can make (utilize Math.floor() here to get a random value -> Reference online documentation)
    // potential_choice_len = potential_choice.length;
    const random_move = Math.floor(Math.random() * potential_choice.length); // Will be used as index for array of potential choices (Using '.length' will simply impl)
    ...
    if (move === undefined) { // Following would deal with #4 operational requirement ('===' does a stricter comparison between operands)
        ...
        // Throw an error for the following
        console.error('Error: The following move is not allowed. The only options you can select are rock, paper, scissors, lizard, or spock!!')
        // throw an error showing that the [ARGUMENT] is out of range (#7 operational requirement).
        throw new RangeError();
    } else {
        // Following would represent a move from the computer
        ...
        switch (move.toLowerCase() ){ // Change spacing here
            ...
            case 'scissors':
                ...
                outcome = (opponent_move === 'paper') ? 'win':
                          ...
                          break;
            case 'rock':
                // Does it matter if we use '' instead of ""?
                outcome = (opponent_move === 'paper') ? 'lose':
                          (opponent_move === 'rock') ? 'tie':
                          (opponent_move === 'scissors') ? 'win':
                          (opponent_move === 'lizard') ? 'win':
...
} // Checkpoint 2: Now, we can see if the RPS and RPSLS game works once we edit one more file/module
